...
def load_video(
    ...
    for size, video in zip(sizes, videos, strict=False):
        ...
        if isinstance(video, str):
            ...
            video_frames, _, info = torchvision.io.read_video(video, pts_unit='sec')
            
            # Convert to [T, C, H, W] (NCHW)
            video_frames = video_frames.permute(0, 3, 1, 2)
            
            # Get timestamps
            pts_seconds, _ = torchvision.io.read_video_timestamps(video, pts_unit='sec')
            # pts_seconds is a list, convert to tensor
            pts_tensor = torch.tensor(pts_seconds)
            
            diffs = (audio_timestamps[None] - pts_tensor[:, None]).abs()
            frame_idxs = diffs.argmin(dim=0)
            frames = video_frames[frame_idxs]
        else:
...
class Processor:
    ...
    def mask_videos(
        ...
        # Helper to load video to NCHW
        def load_v(v):
            if isinstance(v, str):
                v_frames, _, _ = torchvision.io.read_video(v, pts_unit='sec')
                return v_frames.permute(0, 3, 1, 2)
            return v

        video = [load_v(v) for v in videos]
        video_mask = [load_v(v) for v in masks]
...
class SAMAudioJudgeProcessor(Processor):
    ...
    def _load_audio(self, path: str):
        ...
        if sr != self.audio_sampling_rate:
            wav = torchaudio.functional.resample(wav, sr, self.audio_sampling_rate)
        # Ensure 1 channel if needed? AudioDecoder line says num_channels=1
        if wav.shape[0] > 1:
